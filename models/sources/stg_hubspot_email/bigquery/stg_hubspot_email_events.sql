...
with
    source as (select * from {{ var("stg_hubspot_email_stitch_email_events_table") }}),
    renamed as (
        select
            cast(null as string) as list_id,
            concat(
                '{{ var(' stg_hubspot_email_id - prefix ') }}',
                cast(emailcampaignid as string)
            ) as ad_campaign_id,
        ...
        from source e
        left join
            {{ ref("stg_hubspot_crm_contacts") }} c on e.recipient = c.contact_email
