...
with
    ...
    flattened_{{ app }} as (
        ...
        select
            ...
            json_extract_array_element_text(
                required_fields.template_app_link_spec_{{ app }},
                numbers.generated_number::int - 1,
                true
            ) as element
        from required_fields
        inner join
            numbers
            on json_array_length(required_fields.template_app_link_spec_{{ app }})
    ...
    unioned as (
        ...
        union all
        select *
        from extracted_iphone
        union all
        select *
        from extracted_ipad
        union all
        select *
        from extracted_android
