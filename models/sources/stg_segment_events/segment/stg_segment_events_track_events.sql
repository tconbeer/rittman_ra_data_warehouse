            ...
            with
                ...
                renamed as (
                    ...
                    select
                        ...
                        cast(
                            event_text as {{ dbt_utils.type_string() }}
                        ) as event_details,
                        cast(null as {{ dbt_utils.type_string() }}) as page_title,
                        cast(
                            context_page_path as {{ dbt_utils.type_string() }}
                        ) as page_url_path,
                        cast(
                            replace(
                        ...
                        cast(
                            context_page_search as {{ dbt_utils.type_string() }}
                        ) as search,
                        cast(
                            context_page_url as {{ dbt_utils.type_string() }}
                        ) as page_url,
                        cast(
                            {{ dbt_utils.get_url_host("context_page_url") }}
                            as {{ dbt_utils.type_string() }}
                        ) as page_url_host,
                        cast(
                            {{
                                dbt_utils.get_url_parameter(
                                    "context_page_url", "gclid"
                                )
                            }} as {{ dbt_utils.type_string() }}
                        ) as gclid,
                        cast(
                            context_campaign_term as {{ dbt_utils.type_string() }}
                        ) as utm_term,
                        cast(
                            context_campaign_content as {{ dbt_utils.type_string() }}
                        ) as utm_content,
                        cast(
                            context_campaign_medium as {{ dbt_utils.type_string() }}
                        ) as utm_medium,
                        cast(
                            context_campaign_name as {{ dbt_utils.type_string() }}
                        ) as utm_campaign,
                        cast(
                            context_campaign_source as {{ dbt_utils.type_string() }}
                        ) as utm_source,
                        cast(context_ip as {{ dbt_utils.type_string() }}) as ip,
                        cast(
                            anonymous_id as {{ dbt_utils.type_string() }}
                        ) as visitor_id,
