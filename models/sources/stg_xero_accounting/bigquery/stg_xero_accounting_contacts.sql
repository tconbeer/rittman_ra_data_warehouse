        ...
        {% if var("stg_xero_accounting_etl") == "fivetran" %}
            with
                source as (
                    ...
                    select * from {{ source("fivetran_xero_accounting", "contact") }}
                ),
                addresses as (
                    select
                        contact_id,
                ...
                renamed as (
                    select
                        ...
                        as contact_address,
                        {{ fivetran_utils.string_agg("addresses.city", ",") }}
                        as contact_city,
                        {{ fivetran_utils.string_agg("addresses..region", ",") }}
                        as contact_state,
                    ...
                    where
                        concat(contacts.first_name, ' ', contacts.last_name) is not null
                    group by
                        1,
                        2,
                        3,
