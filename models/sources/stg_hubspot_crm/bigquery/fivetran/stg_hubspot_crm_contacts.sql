    ...
    {% if var("crm_warehouse_contact_sources") %}
        {% if "hubspot_crm" in var("crm_warehouse_contact_sources") %}
            {% if var("stg_hubspot_crm_etl") == "fivetran" %}
                ...
                with
                    source as (
                        select * {{ source("fivetran_hubspot_crm", "contacts") }}
                    ),
                    renamed as (
                        select
                            concat(
                                ...
                                cast(
                                    property_associatedcompanyid as string
                                ) as contact_company_id,
                                cast(
                                    property_hubspot_owner_id as string
                                ) as contact_owner_id,
