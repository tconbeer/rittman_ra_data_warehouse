...
with
    source as (
        {{
            filter_stitch_relation(
                relation=source("stitch_hubspot_crm", "deals"), unique_column="dealid"
            )
        }}
    ...
    hubspot_deal_stages as (select * from {{ ref("stg_hubspot_crm_pipeline_stages") }}),
    hubspot_deal_owners as (select * from {{ ref("stg_hubspot_crm_owners") }}),
    renamed as (
        select
            dealid as deal_id,
            concat(
            ...
            timestamp_millis(
                safe_cast(property_delivery_schedule_date.value as int64)
            ...
            case
                when property_deal_components.value like '%Managed Services%'
                then true
                else false
            end as is_managed_services_deal,
            case
                when property_deal_components.value like '%License Referral%'
                then true
                else false
            ...
            case
                when property_deal_components.value like '%Looker%' then true else false
            end as is_looker_skill_requirement,
            case
                when property_products_in_solution.value like '%Segment%'
                then true
                else false
            ...
            case
                when property_products_in_solution.value like '%Snowflake%'
                then true
                else false
            ...
            case
                when property_products_in_solution.value like '%Fivetran%'
                then true
                else false
            ...
            round(
                cast(property_hs_time_in_1164152.value as int64) / 3600000 / 24, 0
            ...
            round(
                cast(property_hs_time_in_qualifiedtobuy.value as int64) / 3600000 / 24,
                0
            ...
            round(
                cast(property_hs_time_in_1357010.value as int64) / 3600000 / 24, 0
            ...
            round(
                cast(property_hs_time_in_1164140.value as int64) / 3600000 / 24, 0
        ...
        from source, unnest(associations.associatedcompanyids) associatedcompanyids
    ),
    joined as (
        select
            d.*,
