...
with
    ...
    renamed as (
        select
            concat(
                '{{ var(' stg_hubspot_crm_id - prefix ') }}', engagement_id
            ) as conversation_id,
            concat(
                '{{ var(' stg_hubspot_crm_id - prefix ') }}', contactids.value
            ) as conversation_user_id,
            concat(
                '{{ var(' stg_hubspot_crm_id - prefix ') }}', contactids.value
            ) as conversation_author_id,
            concat(
                '{{ var(' stg_hubspot_crm_id - prefix ') }}', companyids.value
            ) as company_id,
            ...
            coalesce(
                engagement.type, cast(null as {{ dbt_utils.type_string() }})
            ) as conversation_message_type,
            coalesce(
                metadata.text, cast(null as {{ dbt_utils.type_string() }})
            ) as conversation_body,
            coalesce(
                metadata.subject, cast(null as {{ dbt_utils.type_string() }})
