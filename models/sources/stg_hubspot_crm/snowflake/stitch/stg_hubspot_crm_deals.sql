...
with
    source as (select * from {{ var("stg_hubspot_crm_fivetran_deals_table") }}),
    hubspot_deal_company as (
        select * from {{ var("stg_hubspot_crm_fivetran_companies_table") }}
    ),
    hubspot_deal_pipelines_source as (
        select * from {{ var("stg_hubspot_crm_fivetran_deal_pipelines_table") }}),
    hubspot_deal_property_history as (
        select * from {{ var("stg_hubspot_crm_fivetran_property_history_table") }}),
    hubspot_deal_stages as (
        select * from {{ var("stg_hubspot_crm_fivetran_pipeline_stages_table") }}
    ),
    hubspot_deal_owners as (
        select * from {{ var("stg_hubspot_crm_fivetran_deal_owners_table") }}
    ),
    renamed as (
        select
            deal_id as deal_id,
    ...
    joined as (
        ...
        left outer join
            hubspot_deal_owners u on cast(d.deal_owner_id as int) = u.owner_id
    )
...
with
    ...
    renamed as (
        select
            ...
            case
                when property_deal_components:value::string like '%Services%'
                then true
                else false
            end as is_services_deal,
            case
                when property_deal_components:value::string like '%Managed Services%'
                then true
                else false
            end as is_managed_services_deal,
            case
                when property_deal_components:value::string like '%License Referral%'
                then true
                else false
            end as is_license_referral_deal,
            case
                when property_deal_components:value::string like '%Training%'
                then true
                else false
            ...
            case
                when property_products_in_solution:value::string like '%Segment%'
                then true
                else false
            end as is_segment_skill_requirement,
            case
                when property_products_in_solution:value::string like '%dbt%'
                then true
                else false
            end as is_dbt_skill_requirement,
            case
                when property_products_in_solution:value::string like '%Stitch%'
                then true
                else false
            end as is_stitch_skill_requirement,
            case
                when property_products_in_solution:value::string like '%GCP%'
                then true
                else false
            end as is_gcp_skill_requirement,
            case
                when property_products_in_solution:value::string like '%Snowflake%'
                then true
                else false
            end as is_snowflake_skill_requirement,
            case
                when property_products_in_solution:value::string like '%Qubit%'
                then true
                else false
            end as is_qubit_skill_requirement,
            case
                when property_products_in_solution:value::string like '%Fivetran%'
                then true
                else false
