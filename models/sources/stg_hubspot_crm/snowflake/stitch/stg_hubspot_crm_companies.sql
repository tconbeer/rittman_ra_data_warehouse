...
with
    source as (select * from {{ var("stg_hubspot_crm_fivetran_companies_table") }}),
    renamed as (
        select
            concat('{{ var(' stg_hubspot_crm_id - prefix ') }}', id) as company_id,
            replace(
                replace(replace(property_name, 'Limited', ''), 'ltd', ''), ', Inc.', ''
            ) as company_name,
