...
with
    source as (
        {{
            filter_stitch_relation(
                relation=var("stg_asana_projects_stitch_tasks_table"),
                unique_column="gid",
            )
        }}
    ...
    renamed as (
        select
            concat(
                '{{ var(' stg_asana_projects_id - prefix ') }}', source.gid
            ) as task_id,
            parent.gid as parent_task_id,
            concat(
                '{{ var(' stg_asana_projects_id - prefix ') }}', projects.value.gid
            ) as project_id,
            coalesce(
                concat('{{ var(' stg_asana_projects_id - prefix ') }}', assignee.gid),
                '-999'
            ...
            case
                when
                    case when parent.gid is null then 'Task' else 'Subtask' end = 'Task'
                then 1
        ...
        from source, unnest(projects) projects {{ dbt_utils.group_by(40) }}
    )
select *
from renamed
