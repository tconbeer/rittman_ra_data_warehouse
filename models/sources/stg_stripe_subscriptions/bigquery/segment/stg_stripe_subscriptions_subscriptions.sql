    ...
    source as (
        {{
            filter_segment_relation(
                relation=source("segment_stripe_subscriptions", "subscriptions")
            )
        }}
    ),
    renamed as (
        select
            ...
            concat(
                '{{ var(' stg_stripe_payments_id - prefix ') }}', customer_id
            ) as customer_id,
            concat(
                '{{ var(' stg_stripe_payments_id - prefix ') }}', plan_id
            ) as plan_id,
            discount_id,
            ...
            ) as subscription_last_modified_ts
        from source
    )
select *
from renamed
