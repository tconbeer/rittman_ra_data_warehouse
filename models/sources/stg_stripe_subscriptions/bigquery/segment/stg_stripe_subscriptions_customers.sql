    ...
    {% if "stripe_subscriptions" in var("subscriptions_warehouse_sources") %}
        ...
        with
            source as (
                {{
                    filter_segment_relation(
                        relation=source("segment_stripe_subscriptions", "customers")
                    )
                }}
            ...
            renamed as (
                select
                    concat(
                        'stg_stripe_payments_segment-', metadata_clientreplaceme_user_id
                    ) as customer_id,
                    email as customer_email,
                    description as customer_description,
                    concat(
                        '{{ var(' stg_stripe_payments_id - prefix ') }}', id
                    ) as customer_alternative_id,
                    ...
                    ) as customer_is_enterprise,
                    cast(
                        null as {{ dbt_utils.type_boolean() }}
                    ) as customer_is_delinquent,
                    ...
                    cast(
                        null as {{ dbt_utils.type_timestamp() }}
                    ) as customer_last_modified_date
                from source
            )
        select *
        from renamed
