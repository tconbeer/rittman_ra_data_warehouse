            ...
            (
                select
                    json_extract_path_text(
                        replace(replace(context_external_ids, '[', ''), ']', ''), 'id'
                    ) as user_id,
                    sum(
                        case when regexp_count(products, 'Sample') < 1 then 1 else 0 end
                    ) as non_sample_orders,
                    sum(
                        case when regexp_count(products, 'Sample') > 0 then 1 else 0 end
                    ) as sample_orders
