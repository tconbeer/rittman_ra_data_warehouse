...
with
    products as (
        select products
        from {{ var("stg_segment_shopify_events_segment_checkout_started_table") }}
        where is_valid_json_array(products)
        union all
        select products
        from {{ var("stg_segment_shopify_events_segment_order_completed_table") }}
        where is_valid_json_array(products)
    ...
    products_deduped as (
        select
            ...
            json_extract_path_text(
                replace(replace(products, '[', ''), ']', ''), 'shopify_product_id'
            ) as shopify_product_id,
            json_extract_path_text(
                replace(replace(products, '[', ''), ']', ''), 'shopify_variant_id'
