...
with
    source as (select * from {{ ref("stg_google_ads__final_url_performance_tmp") }}),
            ...
            {{
                fivetran_utils.fill_staging_columns(
                    source_columns=adapter.get_columns_in_relation(
                        ref("stg_google_ads__final_url_performance_tmp")
                    ),
    ...
    url_fields as (
        ...
        select
            *,
            ...
            coalesce(
                {{ dbt_utils.get_url_parameter("final_url", "utm_source") }}, 'google'
            ) as utm_source,
            coalesce(
                {{ dbt_utils.get_url_parameter("final_url", "utm_medium") }}, 'cpc'
            ) as utm_medium,
            coalesce(
                {{ dbt_utils.get_url_parameter("final_url", "utm_campaign") }},
                campaign_name
            ) as utm_campaign,
            coalesce(
                {{ dbt_utils.get_url_parameter("final_url", "utm_content") }},
                ad_group_name
            ) as utm_content,
            ...
            *,
            {{
                dbt_utils.surrogate_key(
                    ["date_day", "campaign_id", "ad_group_id", "final_url"]
                )
