                ...
                with
                            ...
                            {{
                                fivetran_utils.fill_staging_columns(
                                    source_columns=adapter.get_columns_in_relation(
                                        ref(
                                            "stg_google_ads__ad_final_url_history_tmp"
                                        )
                                    ),
                    ...
                    most_recent as (
                        ...
                        select
                            *,
                            row_number() over (
                                partition by ad_id order by updated_timestamp desc
                            )
                    ...
                    url_fields as (
                        ...
                        select
                            *,
                            {{ dbt_utils.split_part("final_url", "'?'", 1) }}
                            as base_url,
                            {{ dbt_utils.get_url_host("final_url") }} as url_host,
                            '/'
                            || {{ dbt_utils.get_url_path("final_url") }} as url_path,
                            {{ dbt_utils.get_url_parameter("final_url", "utm_source") }}
                            as utm_source,
                            {{ dbt_utils.get_url_parameter("final_url", "utm_medium") }}
                            as utm_medium,
                            {{
                            ...
                            {{ dbt_utils.get_url_parameter("final_url", "utm_term") }}
                            as utm_term
                        from most_recent
