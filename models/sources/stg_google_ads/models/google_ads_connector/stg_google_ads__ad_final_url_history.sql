...
with
    base as (select * from {{ ref("stg_google_ads__ad_final_url_history_tmp") }}),
            ...
            {{
                fivetran_utils.fill_staging_columns(
                    source_columns=adapter.get_columns_in_relation(
                        ref("stg_google_ads__ad_final_url_history_tmp")
                    ),
    ...
    most_recent as (
        ...
        select
            *,
            row_number() over (partition by ad_id order by updated_timestamp desc)
