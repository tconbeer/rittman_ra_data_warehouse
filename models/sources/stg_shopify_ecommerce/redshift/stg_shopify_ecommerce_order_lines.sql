...
with
    source as (select * from {{ ref("shopify__order_lines") }}),
    order_lines_tax as (

        select order_line_id, title as tax_type, price as tax_amount, rate as tax_rate
        from shopify.tax_line t
        where
            index = (
                select max(index)
                from shopify.tax_line d
                where d.order_line_id = t.order_line_id
            )

    ),
    joined as (
        select o.*, t.tax_type, t.tax_amount, t.tax_rate
        from source o
        left join order_lines_tax t on o.order_line_id = t.order_line_id
    ),
    renamed as (
        select
