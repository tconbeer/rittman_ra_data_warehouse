...
with
    source as (select * from {{ ref("shopify__customers") }}),
    customer_tags as (select * from {{ source("fivetran_shopify", "customer_tag") }}),
    renamed as (
        select
            created_timestamp,
            default_address_id,
            ...
            {{ fivetran_utils.string_agg("t.value", ",") }} as as customer_tags

        from source c
        left join
            customer_tags t on c.customer_id = t.customer_id
