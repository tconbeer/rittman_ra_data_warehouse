...
with
    source as (select * from {{ ref("shopify__orders") }}),
    order_tags as (select * from {{ source("fivetran_shopify", "order_tag") }}),
    renamed as (
        select
            ...
            new_vs_repeat,
            {{ fivetran_utils.string_agg("t.value", ",") }} as order_tags
        from source o
        left join order_tags t on o.order_id = t.order_id {{ dbt_utils.group_by(n=76) }}
    )
select *
from renamed
