...
with
    source as (
        {{
            filter_stitch_relation(
                relation=source("stitch_harvest_projects", "invoices"),
                unique_column="id",
            )
        }}
    ),
    harvest_invoice_line_items as (
        {{
            filter_stitch_relation(
                relation=source("stitch_harvest_projects", "invoice_line_items"),
                unique_column="id",
            )
        }}
    ),
    harvest_expenses as (
        {{
            filter_stitch_relation(
                relation=source("stitch_harvest_projects", "expenses"),
                unique_column="id",
            )
        }}
    ),
    joined as (
        select
            ...
            row_number() over (
                partition by i.client_id order by i.created_at
            ) as client_invoice_seq_no,
            {{
                dbt_utils.datediff(
                    "date(first_value(i.created_at) over (partition by i.client_id order by i.created_at))",
                    "date(i.created_at)",
                    "MONTH",
                )
            }}
            ...
            amount - ifnull(cast(tax_amount as float64), 0) - ifnull(
                cast(e.total_rechargeable_expenses as float64), 0
            ) as net_amount,
            ...
            ) as license_referral_fee_amount_billed,
                ...
                from
                    (
                        select
                            ...
                            ifnull(
                                (case when kind = 'Service' then amount end), 0
                            ) as services_amount_billed,
                            ifnull(
                                (
                                    case
                                        when kind = 'License Referral Fee' then amount
                                    end
                                ),
                                0
                            ) as license_referral_fee_amount_billed,
                            ifnull(
                                (case when kind = 'Product' then amount end), 0
                            ) as expenses_amount_billed,
                            ifnull(
                                (case when kind = 'Support' then amount end), 0
                            ) as support_amount_billed
        ...
        left outer join
            (
                select invoice_id, sum(total_cost) as total_rechargeable_expenses
    ...
    renamed as (
        select
            concat(
                '{{ var(' stg_harvest_projects_id - prefix ') }}', number
            ) as invoice_number,
            concat(
                '{{ var(' stg_harvest_projects_id - prefix ') }}', company_id
            ) as company_id,
            concat(
                '{{ var(' stg_harvest_projects_id - prefix ') }}', invoice_id
            ) as invoice_id,
            concat(
                '{{ var(' stg_harvest_projects_id - prefix ') }}', project_id
            ) as project_id,
            concat(
                '{{ var(' stg_harvest_projects_id - prefix ') }}', creator_id
            ) as invoice_creator_users_id,
            ...
            license_referral_fee_amount_billed
            as invoice_local_total_licence_referral_fee_amount,
