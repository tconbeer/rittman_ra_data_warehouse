            ...
            with
                ...
                harvest_invoice_line_items as (
                    {{
                        filter_stitch_relation(
                            relation=source(
                                "stitch_harvest_projects", "invoice_line_items"
                            ),
                            unique_column="id",
                        )
                    }}
                ...
                joined as (
                    ...
                    join
                        (
                            select
                                *,
                                case
                                    when taxed then total_amount_billed * .2
                                end as tax_billed
                            from
                                (
                                    select
                                        ...
                                        ifnull(
                                            (
                                                case
                                                    when kind = 'License Referral Fee'
                                                    then amount
                                                end
                                            ),
                                            0
                    ...
                    left outer join
                        (
                            select
                                invoice_id,
                                sum(total_cost) as total_rechargeable_expenses
