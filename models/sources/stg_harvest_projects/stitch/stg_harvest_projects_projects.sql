...
with
    ...
    renamed as (
        select
            concat(
                '{{ var(' stg_harvest_projects_id - prefix ') }}', p.id
            ) as timesheet_project_id,
            concat(
                '{{ var(' stg_harvest_projects_id - prefix ') }}', p.client_id
            ) as company_id,
