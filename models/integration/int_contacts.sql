    ...
    with
        ...
        {% if target.type == "bigquery" %}
            ...
            contact_emails as (
                select
                    contact_name,
                    array_agg(
                        distinct lower(contact_email) ignore nulls
                    ) as all_contact_emails
                from t_contacts_merge_list
                group by 1
            ),
            contact_ids as (
                select
                    contact_name, array_agg(contact_id ignore nulls) as all_contact_ids
                from t_contacts_merge_list
                group by 1
            ),
            contact_company_ids as (
                select
                    contact_name,
                    array_agg(
                        contact_company_id ignore nulls
                    ) as all_contact_company_ids
                from t_contacts_merge_list
                group by 1
            ),
        ...
        {% elif target.type == "snowflake" %}
            ...
            contact_company_ids as (
                select
                    contact_name,
                    array_agg(contact_company_id) as all_contact_company_ids
                from t_contacts_merge_list
                group by 1
            ),
            contact_company_addresses as (
