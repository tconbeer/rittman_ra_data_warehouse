...
with
    companies_pre_merged as (select * from {{ ref("int_companies_pre_merged") }}),
    merged as (
        select
            c.company_name,
            case
                when m.company_name is not null
                then m.all_company_ids
                else c.all_company_ids
            end as all_company_ids,
            c.company_phone,
            c.company_website,
            c.company_industry,
        ...
        left outer join
            (
                select
                    company_name,
                    array(
                        select distinct x from unnest(all_company_ids) as x
                    ) as all_company_ids
                from
                    (
                        select
                            company_name,
                            array_concat_agg(all_company_ids) as all_company_ids
                        from
                            (
                                select *
                                from
                    ...
                    (
                        ...
                        join
                            (
                                select
                                    c1.company_name,
                                    c1f.value::string as all_company_ids
                                from
                        ...
                        join
                            (
                                select
                                    c2.company_name,
                                    c2f.value::string as all_company_ids
                                from
        ...
        where
            c.company_name not in (
                ...
                join
                    (
                        select c2.company_name, c2f.value::string as all_company_ids
                        from
                            {{ ref("int_companies_pre_merged") }} c2,
                            table(flatten(c2.all_company_ids)) c2f
