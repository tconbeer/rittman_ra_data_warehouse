{% if var("finance_warehouse_invoice_sources") %}
    ...
    with
        ...
        all_invoice_ids as (
            select
                invoice_number,
                array_agg(distinct invoice_id ignore nulls) as all_invoice_ids
        ...
        merged as (
            select
                ...
                max(
                    invoice_local_total_revenue_amount
                ) as invoice_local_total_revenue_amount,
                ...
                {% if "harvest_projects" in var(
                    ...
                    max(
                        invoice_local_total_billed_amount
                    ) as invoice_local_total_billed_amount,
                    max(
                        invoice_local_total_services_amount
                    ) as invoice_local_total_services_amount,
                    max(
                    ...
                    ) as invoice_local_total_expenses_amount,
                    max(
                        invoice_local_total_support_amount
                    ) as invoice_local_total_support_amount,
                {% endif %}
        ...
        joined as (
            select
                ...
                a.all_invoice_ids,
                timestamp_diff(
                    invoice_paid_at_ts, invoice_issue_at_ts, day
                ) as invoice_total_days_to_pay,
                30 - timestamp_diff(
                    invoice_paid_at_ts, invoice_issue_at_ts, day
                ) as invoice_total_days_variance_on_payment_terms,
                timestamp_diff(
                    invoice_paid_at_ts, invoice_due_at_ts, day
                ) as invoice_total_days_overdue
            from merged i
