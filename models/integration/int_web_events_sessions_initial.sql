...
{% set window_clause = "     partition by session_id     order by event_number     rows between unbounded preceding and unbounded following     " %}
...
with
    ...
    marketing_channel_mapping as (select * from {{ ref("marketing_channel_mapping") }}),

    channel_mapping as (select * from {{ ref("marketing_channel_mapping") }}),
    agg as (
    ...
    diffs as (
        ...
        select
            ...
            *,
            ...
            {{ dbt_utils.datediff("session_start_ts", "session_end_ts", "SECOND") }}
