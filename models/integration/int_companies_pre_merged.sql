...
with
    ...
    all_company_ids as (
        select
            company_name, array_agg(distinct company_id ignore nulls) as all_company_ids
    ...
    all_company_ids as (
        select company_name, array_agg(distinct company_id) as all_company_ids
        from t_companies_pre_merged
        group by 1
    ),
    all_company_addresses as (
    ...
    joined as (
        select i.all_company_ids, g.*, a.all_company_addresses
        from grouped g
