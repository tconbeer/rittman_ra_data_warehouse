        ...
        converting_sessions_deduped_labelled as (
            select
                ...
                case
                    when
                        c.created_account_ts
                        between s.session_start_ts and s.session_end_ts
                    then true
                    else false
                end as account_opening_session,
                case
                    when
                        (c.converted_ts between s.session_start_ts and s.session_end_ts)
                    then true
                    else false
                ...
                * except (first_page_url_host),
                ...
                case
                    ...
                    then
                        safe_cast(
                            1.1
                            - row_number() over (partition by blended_user_id) as string
                        )
                    when
                    ...
                    then
                        safe_cast(
                            round(
                                1.1
                                - 1 / row_number() over (partition by blended_user_id),
                                2
                            ) as string
                        )
                    else 'null'
