{% if var("finance_warehouse_journal_sources") %}
    ...
    with
        ...
        invoices as (
            ...
            select i.*, all_invoice_ids as invoice_id
            from
                {{ ref("wh_invoices_fact") }} i, unnest(all_invoice_ids) all_invoice_ids

        ),
        bank_transactions as (select * from {{ ref("wh_bank_transactions_fact") }}),
        companies as (select * from {{ ref("wh_companies_dim") }}),
        joined as (
            ...
            select
                ...
                case
                    when journals.source_type in ('ACCPAY', 'ACCREC')
                    then concat('xero-', journals.source_id)
                end as invoice_id,
                case
                ...
                case
                    when journals.source_type in ('TRANSFER')
                    then concat('xero-', journals.source_id)
                end as bank_transfer_id,
                case
                    when journals.source_type in ('MANJOURNAL')
                    then concat('xero-', journals.source_id)
                end as manual_journal_id,
                case
                    when
                        journals.source_type in (
                            'APPREPAYMENT',
                            'APOVERPAYMENT',
                            'ACCPAYPAYMENT',
                            'ACCRECPAYMENT',
                ...
                case
                    when journals.source_type in ('ACCPAYCREDIT', 'ACCRECCREDIT')
                    then concat('xero-', journals.source_id)
                end as credit_note_id

            from journals
            left join accounts on accounts.account_id = journals.account_id
        ...
        first_company as (
            ...
            select
                joined.*,
                coalesce(
                    invoices.company_pk, bank_transactions.contact_id
                ) as company_pk
            from joined
