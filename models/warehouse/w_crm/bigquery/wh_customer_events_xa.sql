...
{% if var("crm_warehouse_company_sources") %}
    {{ config(alias="customer_events_xa", materialized="view") }}
    with
        events as (
            select
                company_pk,
            ...
            {% if var("finance_warehouse_invoice_sources") %}
                ...
                from
                    {{ ref("wh_customer_contacts_360_xa") }},
            ...
            {% if var("crm_warehouse_contact_sources") %}
                ...
                from
                    {{ ref("wh_customer_contacts_360_xa") }}, unnest(contact) as contact
            {% endif %}
            {% if var("crm_warehouse_deal_sources") and var(
            ...
            {% if var("crm_warehouse_conversations_sources") and var(
                ...
                select
                    ...
                    left(
                        replace(
                            regexp_replace(
                                conversations.conversation_body, '[^a-zA-Z0-9]', ' '
                            ),
                            '  ',
                            ' '
                        ),
    ...
    select
        *,
        row_number() over (
            partition by company_pk order by event_ts
        ) as client_event_seq,
        date_diff(
            date(event_ts), min(date(event_ts)) over (partition by company_pk), month
        ) as months_since_client_created,
        min(date(event_ts)) over (partition by company_pk) as client_cohort_month,
        case
            when event_type = 'Invoice Paid'
            then event_value
            when event_type = 'Delivery Cost'
            then -1 * event_value
            else 0
        end as total_ltv
    from events
