...
{% if var("crm_warehouse_contact_sources") and var("crm_warehouse_company_sources") %}
    ...
    select
        ...
        case
            when pricing_views > 0 then 'Buying' else 'Researching'
        end as buying_stage,
        case
            when
                influencer_status is null and (pricing_views > 0 or casestudy_views > 0)
            then 'Prospect'
            when
                influencer_status is null
                and (pricing_views = 0 and casestudy_views = 0)
            then 'Visitor'
            when
