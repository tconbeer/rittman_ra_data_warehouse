{%- macro bootstrap_schemas(staging_schema_name, seed_schema_name, reset) -%}
    ...
    {% if reset %}
        ...
        {{
            drop_schema(
                target.database,
                schema_prefix ~ target.schema ~ "_" ~ staging_schema_name,
            )
        }}
        ...
        {{
            drop_schema(
                target.database,
                schema_prefix ~ target.schema ~ "_" ~ seed_schema_name,
            )
        }}
    ...
    {{
        create_schema(
            target.database, schema_prefix ~ target.schema ~ "_" ~ staging_schema_name
        )
    }}
    {{ create_schema(target.database, schema_prefix ~ target.schema ~ "_logs") }}
    {{
        create_schema(
            target.database, schema_prefix ~ target.schema ~ "_" ~ seed_schema_name
        )
    }}
