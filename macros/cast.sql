...
{% elif target.type == "redshift" %} cast({{ value | default("null") }} as varchar)
{% else %}
...
{% if target.type == "bigquery" %} cast({{ value | default("null") }} as int64)
...
{% if target.type == "bigquery" %} cast({{ value | default("null") }} as float64)
{% elif target.type == "redshift" or target.type == "snowflake" %}
